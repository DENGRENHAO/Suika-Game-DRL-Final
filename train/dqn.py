...
class MemoryEfficientPERBuffer:
    ...
    """
    def __init__(self, capacity, input_shape=(4, 96, 96), alpha=0.6, beta_start=0.4, beta_frames=100000):
        ...
        self.fruit = np.zeros(capacity, dtype=np.uint8)
        self.next_states = np.zeros((capacity, *input_shape), dtype=np.uint8)
        self.next_fruit = np.zeros(capacity, dtype=np.int64)
    ...
    def add(self, state, fruit, action, reward, next_state, next_fruit, done):
        """
        Adds an experience
        """
        ...
        self.fruit[self.data_pointer] = fruit
        self.next_fruit[self.data_pointer] = next_fruit
        self.actions[self.data_pointer] = action
        self.rewards[self.data_pointer] = reward
        self.next_states[self.data_pointer] = next_state
    ...
    def sample(self, batch_size):
        ...
        # --- Retrieve Batch Data ---
        batch_states = self.states[sampled_indices]
        batch_fruit = self.fruit[sampled_indices]
        batch_actions = self.actions[sampled_indices]
        batch_rewards = self.rewards[sampled_indices]
        batch_next_states = self.next_states[sampled_indices]
        batch_next_fruit = self.next_fruit[sampled_indices]
...
class DuelingCNNQNet(nn.Module):
    ...
    def __init__(self, input_shape, n_actions):
        ...
        self.value_stream = nn.Sequential(
            nn.Linear(fc_input_size, 512), # Takes flattened features
            nn.ReLU(),
    ...
    def forward(self, x, fruit):
        ...
        if isinstance(fruit, (np.ndarray, np.generic)): # np.generic covers scalar types like np.int64
            # Ensure it's a tensor and move to the same device as x if needed
            fruit = torch.from_numpy(np.array(fruit)).to(x.device) # Convert to numpy array first for scalars

        if fruit.dim() == 1:
            # If shape is (batch_size,), unsqueeze to (batch_size, 1)
            fruit_features = fruit.unsqueeze(1).float()
        elif fruit.dim() == 0:
            # If fruit is a single scalar, expand it to (batch_size, 1)
            # This assumes `x` is already batched and we need to match its batch size
            batch_size = x.size(0)
            fruit_features = fruit.expand(batch_size, 1).float()
        else:
            # Assume it's already (batch_size, 1) or similar, just ensure float type
            fruit_features = fruit.float()

        combined_features = torch.cat((flattened_features, fruit_features), dim=1)

        # Calculate Value and Advantage streams
        value = self.value_stream(combined_features)      # Shape: (batch_size, 1)
        advantages = self.advantage_stream(combined_features) # Shape: (batch_size, n_actions)
...
class RainbowDQNAgent:
    ...
    def __init__(self,
                 ...
                 target_update_freq=100,# How often to update target network (in steps)
    ...
    def select_action(self, state, fruit, use_epsilon=True):
        """
        ...
        if use_epsilon and random.random() <= self.epsilon:
            ...
            with torch.no_grad():
                q_values = self.q_net(state_tensor, fruit)
    ...
    def store_experience(self, state, fruit, action, reward, next_state, next_fruit, done):
        """
        ...
        self.replay_buffer.add(state, fruit, action, reward, next_state, next_fruit, done)
    ...
    def _compute_td_error_and_loss(self, batch_data, indices, is_weights):
        ...
        fruits = torch.tensor(batch_data['fruits'], dtype=torch.int64).unsqueeze(1).to(self.device) # Shape: (batch, 1)
        ...
        next_fruits = torch.tensor(batch_data['next_fruits'], dtype=torch.int64).unsqueeze(1).to(self.device) # Shape: (batch, 1)
        ...
        with torch.no_grad():
             q_values_next_main = self.q_net(next_states, next_fruits)
        ...
        # Shape: (batch, 1)
        with torch.no_grad():
            q_values_next_target = self.target_net(next_states, next_fruits).gather(1, best_actions_next)
        # --- End Double DQN ---
        ...
        # Shape: (batch, 1)
        current_q_values = self.q_net(states, fruits).gather(1, actions)
...
def run_one_episode(env, agent, is_training=True):
    state, info = env.reset()
    done = False
    total_reward = 0
    total_steps = 0

    state, fruit = state['boards'], state['next_fruit']

    while not done:
        action = agent.select_action(state, fruit, use_epsilon=is_training)
        action /= 10.0
        next_state, reward, done, truncated, info = env.step((action, ))
        next_state, next_fruit = next_state['boards'], next_state['next_fruit']
        agent.store_experience(state, fruit, action, reward, next_state, next_fruit, done)
        state = next_state
        total_reward += reward
        total_steps += 1
...
def pre_eval(env, agent, iter=20):
    total_reward = 0
    for _ in range(iter):
        total_reward += run_one_episode(env, agent, is_training=False)
    average_reward = total_reward / iter
    print("Pre Eval Reward: ", average_reward)
    return average_reward

def main():
    env = CoordSizeToImage(SuikaEnv(n_frames=4))
    agent = RainbowDQNAgent(input_shape=(NUM_STACKED_FRAMES, FRAME_WIDTH, FRAME_HEIGHT), n_actions=11, device=DEVICE)
    
    if os.path.exists(SAVE_PATH):
        agent.load_model(SAVE_PATH)

    best_reward = pre_eval(env, agent, iter=5)
    rewards_history = []
    ...
    for _ in tqdm.tqdm(range(TRAIN_ITER)):
        ...
        if (_ + 1) % 100 == 0 or sum(rewards_history[-5:]) / 5 > max(1000, best_reward):
            average_reward = pre_eval(env, agent)
