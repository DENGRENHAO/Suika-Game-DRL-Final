    ...
    for episode in tqdm(range(NUM_EPISODES)):
        ...
        while not done:
            ...
            agent.replay_buffer.add(
                {
                    "state": state['boards'],
                    "cur_fruit": torch.tensor(state['cur_fruit'], dtype=torch.long),
                    "action": torch.from_numpy(action).float(),
                    "reward": torch.tensor(reward, dtype=torch.float32),
                    "next_state": next_state['boards'],
                    "next_cur_fruit": torch.tensor(next_state['cur_fruit'], dtype=torch.long),
